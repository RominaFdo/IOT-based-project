...
void setup() {
  ...
  if (LoadCell.getTareTimeoutFlag() || LoadCell.getSignalTimeoutFlag()) {
    ...
    LoadCell.setCalFactor(calibrationValue); // set calibration value (float), initial value 1.0 may be used for this sketch
  ...
  digitalWrite(sirenPin, LOW); // Ensure LED is off initially
...
void readReedSensor(){
  ...
  if (WiFi.status() != WL_CONNECTED && currentState == OPEN) {
    digitalWrite(sirenPin, HIGH); // Turn LED on
  } else {
    digitalWrite(sirenPin, LOW); // Turn LED off
  }
...
// void setUserWeight() {
//   Serial.println("Enter your weight (i.e. 60) and send it from serial monitor (kilograms):");

//   boolean _resume = false;
//   while (_resume == false) {
//     if (Serial.available() > 0) {
//       userWeightGrams = body
